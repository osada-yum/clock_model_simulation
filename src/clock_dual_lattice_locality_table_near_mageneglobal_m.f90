module sixclock_periodic_dual_lattice_locality_mageneglobal_m
  ...
  real(real64) :: magne_diff_table(0:mstate-1, 0:mstate-1)
...
contains
  ...
  impure subroutine init_tables()
    ...
    do nc = 0, mstate - 1
       do c = 0, mstate - 1
          magne_diff_table(c, nc) = magne_table(nc) - magne_table(c)
       end do
    end do
    do c = 0, mstate - 1
       do l = 0, mstate - 1
  ...
  impure subroutine local_flip_even(x, y, uy, dy)
    ...
    if (lx < 1) lx = nx / 2
    candidate_states = sixclock_even(x, y) + (1 + floor((mstate - 1) * rnds(1, 2 * x - 1 + iand(y + 1, b'1'), y)))
  ...
  impure subroutine local_flip_odd(x, y, uy, dy)
    ...
    if (lx < 1) lx = nx / 2
    candidate_states = sixclock_odd(x, y) + (1 + floor((mstate - 1) * rnds(1, 2 * x - 1 + iand(y, b'1'), y)))
    if (candidate_states >= mstate) candidate_states = candidate_states - mstate
    prob = probability_table(&
...
end module sixclock_periodic_dual_lattice_locality_mageneglobal_m
